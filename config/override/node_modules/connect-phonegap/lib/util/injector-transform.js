...
module.exports = function(options) {
  ...
  InjectHTML.prototype._transform = function (chunk, encoding, callback) {
    ...
    var cspRegex = /<xmeta.+Content-Security-Policy.+content.+"(.+)".*>/i;
    ...
    if(result != null && result.length>=2) {
      ...
      cspTag = '<xmeta http-equiv="Content-Security-Policy" content="' + cspString + '">';
      newChunk = newChunk.replace(cspRegex, cspTag);

      if(options.emitter) {
        options.emitter.emit('log', '[console.warn]'.yellow + ' mrduguo:DISABLED Content Security Policy has been modified to be:', cspTag);
      }
    } else if(result == null){
      cspObject = {};
      cspString = cspBuilder(cspObject);
      ...
      if(options.emitter) {
        options.emitter.emit('log', '[console.warn]'.yellow + ' mrduguo:DISABLED Content Security Policy has been added:', cspTag);
      }
    }
